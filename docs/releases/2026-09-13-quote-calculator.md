...
## 順帶調整：常平訂單資料區改回權限鍵（2026-09-16）

- 之前（#43）是寫死 email 白名單；改成看後台「團隊管理」的 **`changping_ship`** 勾選，**管理員不會自動通過**，目前只勾給 Snow（10011）。
- 合併後 Snow 要到團隊管理把自己勾上「常平訂單資料區 (Changping POC，限 Snow)」並重新登入，否則首頁卡片與 `/changping-ship` 會看不到（API 回 404）。

## 預設參數調整（2026-09-16，Snow 指示）

- 常平廠三個品項（鑰匙圈／貼合鑰匙圈／登山鉤）預設 **耗損率 15%、毛利率 27%**（成本率 0.73）；原本各自是 10%／28%、10%／30%、10%／35%。
- 對報價的影響（同一張單）：成本 ×1.15/1.10 ＝ +4.55%；報價再 ×(舊成本率/0.73)。以貼合款為例（原 30%）報價約 −0.2%，鑰匙圈（原 28%）約 +3.1%，登山鉤（原 35%）約 −6.9%。
- seed 與 SQL 已同步；正式庫三筆 quote_products.config 已直接更新（golden 不受影響，各自帶自己的耗損與成本率）。

## Golden 稽核備註（2026-09-17）

- `quote_golden_cases` 加 `audit_note` 欄：每筆驗證案例在後台名稱底下顯示「可信度（高／中高／中）、來源檔、手改處、核可建議」，Snow 核可時不用回頭翻 Excel。綠字＝可信度高、琥珀字＝不建議核可（例如掛在鑰匙圈下的 ANDY 畫板）。
- 後台 Excel 匯入的新案例自動標「尚未稽核」。
- 正式庫要跑 `sql/20260917_quote_golden_audit_note.sql`（加欄位＋回填 8 筆，可重複執行）。

## 材料價格表後台調整（2026-09-17，Snow 回饋）

- 單價欄改純數字框（沒有上下箭頭，避免誤點）、加寬到看得到整個數字；**單價為必填**，空白或非數字會紅框並擋儲存。
- 欄寬重排：名稱縮、ARGO 料號放大、ERP 建議價縮小、備註固定寬。
- 每個分組（板材／PET／五金／包材／工序…）標題右側「＋ 新增項目」可手動新增：名稱（引擎鍵）與單價必填；板材必填套版寬／高（前台拼板用）。新增列生效日自動帶今天、來源標「手動新增」；同廠別同名會擋。新增後要到「品項維護」把它加進品項選項才用得到。

## Excel 匯入 → 建立新品項（2026-09-20）

- 匯入頁多了「2b. 用這份 Excel 建立新品項」：從 `主产品` 分頁反推品項設定（主板、第二板、印刷方式與 PET、貼合／清洗、切割時間、包裝產能、配件、包裝），你只填台灣品名，代碼自動產生，建好是草稿；各數量分頁自動成為該品項的 golden（待核可）。
- 材料價格表與全域參數仍是唯一真相：新品項耗損／成本率一律套常平廠預設 15%／0.73，Excel 上手打的單價只拿來比對，前台算價一律查價格表；價格表缺的品名套用時一併新增（用 Excel 上的價）。
- 「參考與核對」：自動找結構相近的既有品項（同主板／同印刷／單雙板）當參考，核對切割時間、包裝產能、耗損、成本率、各項單價（Excel vs 價格表），加上解析警告，逐條列出「異常／提醒」。
- 本機 seed 模式只能預覽、按「套用」會被擋（避免寫進正式庫）。

## 匯入流程定案（2026-09-20，Snow）

- 後台四區：品項維護／材料價格表／全域參數／Excel 匯入。**材料價格表與全域參數是唯一真相**。
- 品項維護拿掉「＋ 新增品項」（API create 保留），新品項一律由 Excel 匯入建立。
- 匯入頁多「2c. 全域參數差異」：Excel 的人工／折舊／刀費／包裝人力／拼板間距 vs 後台現值，**預設全不勾**，勾了才改後台（全品項共用）；材料價格差異維持預設勾差異。
- 主产品手填、價格表沒有的品名，建立新品項時自動新增（用 Excel 上的價）。
- 一次性作業：全資料夾 21 份常平表掃描結果 `常平報價表全掃描_新品名與價格差異_20260920.xlsx`（14 個新品名、12 個價格不一致），Snow 審完再寫入正式價格表。

## 掃描結果登入與紙卡階梯價（2026-09-22，Snow 指示）

- 價格不一致：板材／炫彩膜等一律用最新版本的價——比對後現價已全部等於最新檔，不動。
- 全掃描 14 個新品名已寫進正式價格表（來源標「常平報價表全掃描 2026-09-20」）；seed／SQL 同步（195 項）。
- 紙卡（纸卡 4c／纸卡／纸卡无印刷）每次尺寸數量都不同：改為**階梯價＋歷史參考**——前台勾選後列出歷史「數量 → 單價（案名）」小標籤，點一下帶入，報價者自己填；價格表存 `attrs.reference_history`。鑰匙圈／貼合鑰匙圈配件清單多了 纸卡、纸卡无印刷（預設不勾）。
