...
import {
  ...
  toErpPayload,
  parseSoDateDigits,
  mapMoExportRowsT as mapAllToExport,
  type ExportRow,
  type SoMatchResult,
  ...
  const runPostImportSync = useCallback(async (factory: 'T' | 'C' | 'O', sheetDate: string) => {
    ...
    try {
      ...
      rows = rows.map(r => {
        ...
        // 「唯一製令」保底 fallback：不再只憑「這批只查得到一張製令」就套用，必須這張製令
        // 實際登記的數量跟這一列要的數量吻合、且末兩碼等於項號，否則同品號同訂單如果出現
        // 數量不同的新增/追加列，會被誤套用到別列早就完工的製令上（2026-08-26 發現：
        // SO260805024 項次4 的 1 片客戶備品被誤套用成項次2 那張數量8、已繳庫完工的製令）。
