...
// 從檔名裡找「訂單號 + #項號」，例如：
//   SO260805024#1.jpg / SO260805024#01-v2.png（可有零填充與後綴，不需完全相符）
//   【商品示意圖】SO260910017_#1A.pdf —— 單號與 # 之間夾底線，項號後面再接 A/B/C/D
//   SOB260806505-#2.jpg / SOA260810-090811-235#3.pdf
//
// 三個放寬的地方（2026-09-11 使用者回報「這種命名方式抓不到」）：
//   1. 單號與 # 之間允許底線／空白／點／連字號（原本要求 # 緊接在數字後面）
//   2. 前綴不再限定 SO＋數字——SOA／SOB／RO 都要認得（同 2026-09-02 出單表那次修正：
//      近 60 天的單號有 28% 是 SOB/SOA 開頭，用 /SO\d+/ 會整批靜默跳過）
//   3. 項號後面允許接一個英文字母：同一個項次有多張圖時，設計部用 #1A #1B #1C #1D 區分，
//      A/B/C/D 只是「同一項次的第幾張」，一律歸到同一個項次，列印時依檔名排序全部插入
//
// 負向前瞻 (?!\d) 維持不變，避免 #1 誤配到 #10、#11 這種同前綴不同項次的檔名。
// 單號本身結尾一定是數字（SOA260810-090811-235），不能把後面當分隔用的 - 或 _ 吃進單號，
// 否則組出來的 key 會變成「SOB260806505-#2」而永遠對不到列印頁查詢用的 key。
const ORDER_LINE_PATTERN = /((?:SO[A-Z]?|RO)\d(?:[\d-]*\d)?)[\s_.-]*#\s*0*(\d+)(?!\d)/i
 ...
 * 一個訂單#項號可能對到多個檔案（同時有圖跟 PDF，或 #1A~#1D 這種同一項次多張圖），
 * 全部保留、列印時依序全部插入。
 */
